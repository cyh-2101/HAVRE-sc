"""Smoke reload and formal-readiness gate for the Stage 9A v6 candidate."""
...
from mlsys.training.stage9a_real_v6 import (  # noqa: E402
    FORMAL_RUN_ID,
    RENDERED_ROOT,
    SMOKE_RUN_ID,
    TRAINING_PLAN_PATH,
...
from mlsys.training.stage9a_unseen_v6 import load_and_verify_unseen
...
def _reload_adapter(*, run_id: str, formal: bool, report_path) -> dict:
    """Prove one immutable v6 adapter reloads over the exact local base."""
    import torch
    ...
    if binding.get("formal") is not formal or binding.get("run_id") != run_id:
        raise ValueError("v6 reload received an adapter from the wrong run boundary")
    ...
    if (
        ...
        raise ValueError("v6 reload training/adapter binding mismatch")
    if report_path.exists():
        raise FileExistsError(f"immutable v6 reload evidence already exists: {report_path}")
    ...
    try:
        ...
        if not output:
            raise RuntimeError("v6 smoke adapter reload produced an empty output")
    ...
    if failure is None and not _resource_evidence_is_within_stage9a_limits(
        ...
        failure = RuntimeError("v6 smoke reload resource evidence is outside limits")
...
def require_smoke_readiness() -> dict:
    ...
    if (
        ...
        raise ValueError("v6 smoke/reload/unseen formal-readiness binding mismatch")
    return {"training": training, "adapter": adapter, "reload": reload, "unseen": unseen}
